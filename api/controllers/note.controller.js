...
module.exports.get_all_note = (req, res, next)=>{
    ...
    let params = {uid};
    let query = `
    match (a:Acc)-[:Linked]->(n:Cont{type:'note'})-[l:Has{commit:last(n.commitList)}]->(x:Prop)
       where id(a)= $uid
       return case
...
module.exports.get_note_detail = (req, res, next)=>{
    ...
    let query = `
      match (a:Acc)-[l1:Linked*]->(n:Cont)-[ly:Has{commit:last(n.commitList)}]->(y:Prop:Title)-[lx:Has*{commit:last(n.commitList)}]->(x:Prop)
      ...
      return {
        main: data[0].property.map(x=>{
           return {
              ...
              label:x.labels.filter( p => { return p != 'Prop' })[0]
           };
        }),
        container: {
        ...
        title: {
          ...
          label:data[0].title.labels.filter( p => { return p != 'Prop' })[0]
        }
      };
    })
    .then( data => {
...
module.exports.update = (req, res, next)=>{
  ...
  let Q2 = `
    match(c:Cont) where id(c) = $cont
    set c.commitList =  c.commitList + $now
    create (new:Prop:${params.label}{value:$value}) return new
  `;
  ...
  .then( () =>{
    let first = Q1_data.last.lastlist[0].labels.filter(x => { return x != 'Prop'})
    if(first[0] != 'Title' ){
      lastlist.reverse();
    }
  })
  ...
  .then( data => {
    ...
    Q1_data.last.lastlist.map(x => {
      ...
      if(i == ps.id && ps.label == 'Title' &&
          x.labels.filter(y => {return y != 'Prop'})[0] != 'Title'){
        let ff = x.labels.filter(y => {return y != 'Prop'})[0]
  ...
  .then( data => {
    let f = data[0];
    f.id = f.identity.low;
    f.label = f.labels.filter(l => { return l != 'Prop' })[0];
...
module.exports.add_property = (req, res, next)=>{
  ...
  let Q1 = `
     ...
     call apoc.do.when(
        count <> 0,
        "match (c:Cont)-[:Has{commit:com}]->(t:Title)"+
        "-[:Has*{commit:com}]->(p:Prop) where c=co return collect(p) as list",
        "", {com:last(c.commitList), co:c}
     ) yield value
     return value
  `;
  let Q2_1 =
    `match (c:Cont)-[:Has{commit:last(c.commitList)}]->(t:Title)
    where id(c)= $cont`;
  let Q2_2 =
    ` set c.commitList = c.commitList + $now
    create (new:Prop:Undefined{value:''})
  ...
  .then( data => {
    ...
    return {
      id: f.identity.low,
      value: '',
      label: f.labels.filter(p => { return p != 'Prop'})[0]
    };
  })
  .then( data => {
    utils.commit(tx, res, {uid, data});
...
module.exports.delete_property = (req, res, next)=>{
  ...
  .then( data => {
    ...
    data.map(p => {
      ...
      if(p.labels.filter(l=>{ return l != 'Prop'})[0] == 'Title'){
        Q2_1 += ` match (t:Title) where id(t)=${i}`;
        Q2_2 += `-[:Has{commit:$now}]->(t)`;
      }else if(i != ps.property_id){
        Q2_1 += ` match (x${i}:Prop) where id(x${i})=${i}`;
        Q2_2 += `-[:Has{commit:$now}]->(x${i})`;
      }
    });
  })
...
module.exports.drop_property = (req, res, next)=>{
  // Since it ain't possible to parse the commit, it'll be the last by default
    ...
    let Q1 = `
       match (a:Acc)-[l:Linked]->(n:Cont)
       where id(a) = $uid and id(n) = $cont
       return count(l)
    `;

    let Q2 = `
       match (n:Cont)-[l:Has*{commit:last(n.commitList)}]->(p:Prop)
       where id(n) = $cont
       return collect(p)
    `;

    let Q3_1 = `
      match (n:Cont) where id(n) = $cont
    `;
    let Q3_2 = ` create (n)`;
    ...
    .then( data => {
      ...
      let titleIndex;
      f.map(x=>{
        x.labels.filter(x => x != 'Prop')[0] == 'Title' ? titleIndex = j : null
        j++;
      });
      let title = f.splice(titleIndex, 1);
      let i = title[0].identity.low;
      Q3_1 += ` match (x${i}:Prop) where id(x${i}) = ${i}`;
      Q3_2 += `-[:Has{commit:$now}]->(x${i})`;
      return f;
    })
    .then( f => {
    ...
    .then( f => {
      ...
      f.map( x => {
        ...
        if(i == ps.property_id && ps.direction == 'down'){
          ...
          Q3_1 += `
          match (x${i}:Prop) where id(x${i}) = ${i}
          match (x${previous}:Prop) where id(x${previous}) = ${previous}`;
          Q3_2 += `-[:Has{commit:$now}]->(x${i})-[:Has{commit:$now}]->(x${previous})`;
          previous = 0;
        }else if(previous){
          Q3_1 += ` match (x${previous}:Prop) where id(x${previous}) = ${previous}`;
          Q3_2 += `-[:Has{commit:$now}]->(x${previous})`;
          previous = i;
        }else if(todrop){
          Q3_1 += `
          match (x${i}:Prop) where id(x${i}) = ${i}
          match (x${todrop}:Prop) where id(x${todrop}) = ${todrop}`;
